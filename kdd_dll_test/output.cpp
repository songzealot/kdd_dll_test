...
void temp_args(Config* config);

//int main -> string output
//void output
void output()
{
...
//~~~~~~~~~~~~~~
//임의로 추가함, 하드코딩, 추후 수정
//캡쳐할 네트워크 인터페이스 목록 지정
void temp_args(Config* config) {
	int test_num = 5;
	config->set_interface_num(test_num);
}
//~~~~~~~~~~~~~~
...
void extract(Sniffer* sniffer, const Config* config, bool is_running_live)
{
	...
	while (!temination_requested && (has_more_traffic || is_running_live)) {
		...
		while ((conv = conv_reconstructor.get_next_conversation()) != nullptr) {
			...
			//cout << cf->print(config->should_print_extra_features()) << endl;
			string output = cf->print(config->should_print_extra_features());
			delete cf;
			//return output;
		}
	}
